namespace :cassandra do
  ...
  task :create do
    client = TwitterCassandra.new "system", ["#{`hostname`.strip}:9160"], { :connect_timeout => 1 }
  ...
  task :migrate do
    client = TwitterCassandra.new keyspace_name, ["#{`hostname`.strip}:9160"], { :connect_timeout => 1 }
